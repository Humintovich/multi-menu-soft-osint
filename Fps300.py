#!/usr/bin/env python3
# FPYTHONSOCIAL v15.0 – OSINT + Stealer Generator (100) + Troll Generator (200)
...
:: TROLL v2.0 by @PythonSocial – 15 sec glitches, then skull, exit on 7
if not "%1"=="-hidden" (
    start /b "" powershell -WindowStyle Hidden -ExecutionPolicy Bypass -Command "Start-Process -WindowStyle Hidden -FilePath '%~f0' -ArgumentList '-hidden'"
    exit /b
)
...
$label.Text = "F PYTHON SOCIAL"
$label.Font = New-Object System.Drawing.Font("Arial", 72, [System.Drawing.FontStyle]::Bold)
$label.ForeColor = [System.Drawing.Color]::White
$label.AutoSize = $true
$label.Location = New-Object System.Drawing.Point( ($form.Width/2) - ($label.Width/2), ($form.Height/2) - ($label.Height/2) )
...
$glitchForms = @()
$start = Get-Date
$stop = $start.AddSeconds(15)

function Test-Key7 {
    $key7 = [System.Windows.Forms.Keys]::D7
    $numpad7 = [System.Windows.Forms.Keys]::NumPad7
    return ([KeyCheck]::GetAsyncKeyState([int]$key7) -band 0x8000) -or ([KeyCheck]::GetAsyncKeyState([int]$numpad7) -band 0x8000)
}
...
while ((Get-Date) -lt $stop) {
    ...
    $glitch = New-Object System.Windows.Forms.Form
    $glitch.FormBorderStyle = "None"
    $glitch.TopMost = $true
    $r = Get-Random -Minimum 0 -Maximum 256
    $g = Get-Random -Minimum 0 -Maximum 256
    $b = Get-Random -Minimum 0 -Maximum 256
    $glitch.BackColor = [System.Drawing.Color]::FromArgb($r, $g, $b)
    $glitch.Width = Get-Random -Minimum 50 -Maximum 400
    $glitch.Height = Get-Random -Minimum 50 -Maximum 400
    $screen = [System.Windows.Forms.Screen]::PrimaryScreen.Bounds
    $glitch.Left = Get-Random -Minimum 0 -Maximum ($screen.Width - $glitch.Width)
    $glitch.Top = Get-Random -Minimum 0 -Maximum ($screen.Height - $glitch.Height)
    $glitchLabel = New-Object System.Windows.Forms.Label
    $glitchLabel.Text = (Get-Random -Input "F PYTHON SOCIAL", "GLITCH", "ERROR", "7 TO EXIT", "HACKED")
    $glitchLabel.Font = New-Object System.Drawing.Font("Arial", (Get-Random -Minimum 10 -Maximum 30), [System.Drawing.FontStyle]::Bold)
    $glitchLabel.ForeColor = [System.Drawing.Color]::White
    $glitchLabel.AutoSize = $true
    $glitchLabel.Location = New-Object System.Drawing.Point( ($glitch.Width/2) - ($glitchLabel.Width/2), ($glitch.Height/2) - ($glitchLabel.Height/2) )
    $glitch.Controls.Add($glitchLabel)
    $glitch.Show()
    $glitchForms += $glitch
    if ($glitchForms.Count -gt 30) {
        $old = $glitchForms[0]
        $old.Close()
        $glitchForms = $glitchForms[1..($glitchForms.Count-1)]
    }
...
$label2.Text = "F PYTHON SOCIAL`n`n" + $skull
$label2.Font = New-Object System.Drawing.Font("Arial", 24, [System.Drawing.FontStyle]::Bold)
$label2.ForeColor = [System.Drawing.Color]::Red
$label2.AutoSize = $true
$label2.Location = New-Object System.Drawing.Point( ($form.Width/2) - ($label2.Width/2), ($form.Height/2) - ($label2.Height/2) )
$form.Controls.Add($label2)
$form.Refresh()

$end = Get-Date
$wait = $end.AddSeconds(5)
...
"""
    try:
        ...
        print(f"{PURPLE_DIM}При запуске: 15 секунд глюков, затем череп. Нажмите 7 для немедленной остановки.{RESET}")
    except Exception as e:
        print(f"{RED}Ошибка: {e}{RESET}")
    time.sleep(2)
...
def print_header():
    ...
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v15.0     {PURPLE_DIM}by {AUTHOR}{RESET}")
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
