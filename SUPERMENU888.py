#!/usr/bin/env python3
import subprocess, sys, importlib, os, time, random, shutil, re, json, base64, hashlib, sqlite3, csv
...
def key_screen():
    ...
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v27.0{RESET}")
    ...
    def search(self, query, search_type='all'):
        ...
        for record in self.data:
            if isinstance(record, dict):
                # Поиск по всем значениям словаря
                match = False
                for key, value in record.items():
                    ...
                    if search_type == 'phone':
                        ...
                        if '@' in str(value) and query_lower in str(value).lower():
                            match = True
                            break
                        ...
                        if any(x in str(value).lower() for x in ['ул','пр','д','кв']) and query_lower in str(value).lower():
                            match = True
                            break
                    else:
                        if query_lower in str(value).lower():
...
def format_record(record):
    if isinstance(record, dict):
        # Определяем ключи для разных полей
        labels = {
            'name': ['фио', 'имя', 'фамилия', 'name', 'fullname', 'fio'],
            'phone': ['телефон', 'phone', 'tel', 'mobile', 'номер'],
            'email': ['email', 'почта', 'mail'],
            'address': ['адрес', 'address', 'место', 'location'],
            'passport': ['паспорт', 'passport', 'серия', 'snils'],
            'birth': ['дата', 'birth', 'рождение', 'date'],
            'inn': ['инн', 'inn'],
            'snils': ['снилс', 'snils']
        }
        out = []
        for key, value in record.items():
            ...
            key_lower = key.lower()
            label = None
            for category, variants in labels.items():
...
def print_header():
    ...
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v27.0     {PURPLE_DIM}от {AUTHOR}{RESET}")
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
