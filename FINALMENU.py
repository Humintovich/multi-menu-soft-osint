#!/usr/bin/env python3
import subprocess, sys, importlib, os, time, random, shutil, re, json, base64, hashlib, requests, whois, threading
...
def check_and_install():
    required = ['requests', 'whois', 'phonenumbers', 'beautifulsoup4']
    for pkg in required:
        try:
...
def key_screen():
    ...
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v21.0{RESET}")
...
def play_music(filepath):
    ...
    if ext not in ['.mp3', '.wav', '.ogg', '.flac', '.m4a', '.aac']:
        return "Неподдерживаемый формат. Поддерживаются: mp3, wav, ogg, flac, m4a, aac"
    try:
        if sys.platform.startswith('win'):
            os.startfile(filepath)
            return "Музыка открыта в стандартном плеере."
        elif sys.platform.startswith('darwin'):
            subprocess.run(['open', filepath])
            return "Музыка открыта в стандартном плеере."
        else:
            if shutil.which('ffplay'):
                subprocess.Popen(['ffplay', '-nodisp', '-autoexit', filepath], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                return "Воспроизведение через ffplay (нажмите Ctrl+C для остановки)."
            else:
                return "ffplay не найден. Установите: pkg install ffplay или используйте другой плеер."
    except Exception as e:
        return f"Ошибка воспроизведения: {e}"

def print_header():
    sys.stdout.write(CLEAR)
    # Разбиваем череп на строки и выводим справа
    skull_lines = RED_SKULL.split('\n')
    logo_lines = LOGO.split('\n')
    max_skull_width = max(len(line) for line in skull_lines) if skull_lines else 0
    
    # Выводим LOGO и череп рядом
    for i in range(max(len(logo_lines), len(skull_lines))):
        logo_part = logo_lines[i] if i < len(logo_lines) else ""
        skull_part = skull_lines[i] if i < len(skull_lines) else ""
        print(f"{PURPLE}{logo_part}{RESET}{' ' * (max_skull_width - len(skull_part))}{RED}{skull_part}{RESET}")
    
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v21.0     {PURPLE_DIM}от {AUTHOR}{RESET}")
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
...
def menu_loop():
    ...
    while True:
        ...
        items = [
            ...
            "29.Пароль-утечки", "30.Комбо", "31.Web-поиск", "32.Спам-кодом", "33.Музыка"
        ]
        ...
        if choice == '0':
            ...
            filepath = input("Введите путь к аудиофайлу: ").strip()
            if filepath:
                print(play_music(filepath))
            input("Нажмите Enter...")
            continue
